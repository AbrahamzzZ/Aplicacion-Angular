...
CREATE PROCEDURE PA_PRODUCTOS_COMPRADOS_PROVEEDOR
AS 
BEGIN
    SELECT TOP 5 P.NOMBRE_PRODUCTO, SUM(DC.ID_PRODUCTO) AS CANTIDAD_COMPRADA FROM DETALLE_COMPRA DC
    INNER JOIN PRODUCTO P ON DC.ID_PRODUCTO = P.ID_PRODUCTO GROUP BY P.NOMBRE_PRODUCTO ORDER BY CANTIDAD_COMPRADA DESC;
END;
GO

CREATE PROCEDURE PA_PROVEEDOR_PREFERENCIA
AS 
BEGIN
    SELECT TOP 5 P.NOMBRES, COUNT(C.ID_PROVEEDOR) AS COMPRAS_TOTALES FROM COMPRA C
    INNER JOIN PROVEEDOR P ON C.ID_PROVEEDOR = P.ID_PROVEEDOR GROUP BY P.NOMBRES ORDER BY COMPRAS_TOTALES DESC;
END;
GO

CREATE PROCEDURE PA_TRANSPORTISTA_VIAJES_REALIZADOS
AS 
BEGIN
    SELECT TOP 5 T.NOMBRES, COUNT(C.ID_TRANSPORTISTA) AS VIAJES_REALIZADOS FROM COMPRA C
    INNER JOIN TRANSPORTISTA T ON C.ID_TRANSPORTISTA = T.ID_TRANSPORTISTA GROUP BY T.NOMBRES ORDER BY COUNT(C.ID_TRANSPORTISTA) DESC;
END;
GO
...
BEGIN
    ...
    INNER JOIN PRODUCTO P ON DV.ID_PRODUCTO = P.ID_PRODUCTO GROUP BY P.NOMBRE_PRODUCTO;
END;
GO
...
AS 
BEGIN
    SELECT TOP 5 CL.NOMBRES, COUNT(V.ID_CLIENTE) AS COMPRAS_TOTALES FROM VENTA V
    INNER JOIN CLIENTE CL ON V.ID_CLIENTE = CL.ID_CLIENTE GROUP BY CL.NOMBRES;
END;
GO

CREATE PROCEDURE PA_VENTA_EMPLEADO
AS 
BEGIN
