...
AS
BEGIN
	SELECT ID_PRODUCTO, P.CODIGO, DESCRIPCION, NOMBRE_PRODUCTO, C.ID_CATEGORIA, C.CODIGO AS CODIGO_CATEGORIA, C.NOMBRE_CATEGORIA, C.ESTADO AS ESTADO_CATEGORIA, PAIS_ORIGEN, STOCK, PRECIO_COMPRA, PRECIO_VENTA, P.ESTADO FROM PRODUCTO P
...
AS
BEGIN
	SELECT ID_PRODUCTO, P.CODIGO, DESCRIPCION, NOMBRE_PRODUCTO, C.ID_CATEGORIA, C.CODIGO AS CODIGO_CATEGORIA, C.NOMBRE_CATEGORIA, C.ESTADO AS ESTADO_CATEGORIA, PAIS_ORIGEN, P.ESTADO FROM PRODUCTO P
	inner join CATEGORIA C on P.ID_CATEGORIA = C.ID_CATEGORIA
...
BEGIN
    SELECT p.NOMBRE_PRODUCTO AS PRODUCTOS, dv.PRECIO_VENTA, dv.CANTIDAD, dv.SUBTOTAL, dv.DESCUENTO FROM DETALLE_VENTA dv
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Insertar cabecera
        INSERT INTO VENTA (ID_USUARIO, TIPO_DOCUMENTO, ID_CLIENTE, NUMERO_DOCUMENTO, MONTO_PAGO, MONTO_CAMBIO, MONTO_TOTAL, DESCUENTO)
        VALUES (@ID_USUARIO, @TIPO_DOCUMENTO, @ID_CLIENTE, @NUMERO_DOCUMENTO, @MONTO_PAGO, @MONTO_CAMBIO, @MONTO_TOTAL, @DESCUENTO);

        DECLARE @ID_VENTA INT = SCOPE_IDENTITY();

        -- Validar stock disponible
        IF EXISTS (SELECT 1 FROM PRODUCTO P INNER JOIN @DETALLE_VENTA D ON P.ID_PRODUCTO = D.IdProducto WHERE P.STOCK < D.Cantidad)
        BEGIN
